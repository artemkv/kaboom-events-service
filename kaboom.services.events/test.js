...
describe('[REST Api Test Suite]', function () {
    ...
    it(':( Try accessing non-existing page', function (done) {
        request.get('http://localhost:8000/xxx', function (error, response, body) {
            expect(response.statusCode).to.equal(404);
    ...
    it(':( Handle error', function (done) {
        request.get('http://localhost:8000/error', function (error, response, body) {
    ...
    it(':( Handle REST error', function (done) {
        request.get('http://localhost:8000/resterror', function (error, response, body) {
